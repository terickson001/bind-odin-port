...
get_coff_symbols_dll :: proc(file: os.Handle, lib: ^Lib) {
	...
	os.read_ptr(
		file,
		&directories,
		int(win_opt_header.num_data_dirs * size_of(Image_Data_Directory)),
	)
...
get_coff_symbols_import_long :: proc(file: os.Handle, lib: ^Lib, offset: u32) {
	...
	for i in 0 ..< header.num_symbols {
		...
		if symbol.name.zeroes != 0 // Short Name
		{
			...
			// fmt.printf("LIB(L.S): %s\n", cstring(raw_data(symbol.name.str[:])))
		} else {
			add_symbol(lib, string_table[symbol.name.offset - 4:])
			// fmt.printf("LIB(L.L): %s\n", cstring(raw_data(string_table[symbol.name.offset - 4:])))
		}
	}
	delete(string_table)
}
