...
parse_file :: proc(using p: ^Parser) {
	...
	for tokens != nil {
		...
		if n == nil {
			lex.error(
				tokens,
				"Top level element is neither a declaration, nor function definition. Got %q",
				tokens.text,
			)
			os.exit(1)
		}
...
parse_unary_expr :: proc(using p: ^Parser, parent_is_sizeof := false) -> ^Node {
	...
	case .Add, .Sub, .Mul, .Inc, .Dec, .Not, .BitAnd, .BitNot:
		op := advance(p)
	...
	case .OpenParen:
		...
		if type != nil {
			...
			if parent_is_sizeof {
				...
				return ast.make(
					ast.Cast_Expr{{}, open, close, type, parse_unary_expr(p, is_sizeof)},
				)
			}
		}
		tokens = open
...
parse_decl_spec :: proc(using p: ^Parser) -> ^Node {
	...
	if allow(p, .OpenParen) != nil {
		...
		case "deprecated":
			arg = parse_string(p)
		case:
			lex.error(tok, "'__declspec(%s)' takes no arguments", ast.ident(name))
		}
		expect(p, .CloseParen)
	}
	close := expect(p, .CloseParen)
...
parse_integer_or_float_type :: proc(using p: ^Parser) -> ^Node {
	...
	if float {
		...
		{
		case 0:
			name = fmt.aprintf("float")
		case 1:
			name = fmt.aprintf("double")
		case 2:
			name = fmt.aprintf("double") // @note(Tyler): ignoring long double
		}
	...
	if unsigned {
		...
		{
		case 0:
			name = fmt.aprintf("uint")
		case 1:
			name = fmt.aprintf("ulong")
		case 2:
			name = fmt.aprintf("ulonglong")
		}
		return ast.make(ast.Numeric_Type{{}, start, name})
	}
	...
	{
	case 0:
		name = fmt.aprintf("int")
	case 1:
		name = fmt.aprintf("long")
	case 2:
		name = fmt.aprintf("longlong")
	}
	return ast.make(ast.Numeric_Type{{}, start, name})
}
...
parse_type_spec :: proc(using p: ^Parser, var_name: ^^Node, cc: ^^Token) -> ^Node {
	...
	case .___cdecl, .___clrcall, .___stdcall, .___fastcall, .___thiscall, .___vectorcall:
		token := advance(p)
...
parse_type :: proc(using p: ^Parser, var_name: ^^Node, check_type_table := false) -> ^Node {
	...
	case .Ident:
		...
		if ast.ident(base_type) != "void" &&
		   check_type_table &&
		   ast.ident(base_type) not_in type_table {
			tokens = reset
			return nil
		}
	...
	if calling_convention != nil {
		...
		case:
			lex.error(
				calling_convention,
				"Calling convention %q used on non-function type",
