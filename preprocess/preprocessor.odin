...
make_preprocessor :: proc() -> ^Preprocessor {
	...
	pp.include_dirs = make(
		[]string,
		len(config.global_config.include_dirs) + len(lib.sys_info.include),
	)
...
parse_macro_arg :: proc(using pp: ^Preprocessor, param: ^Macro_Param) -> ^Macro_Arg {
	...
	for !(((tokens.kind == .Comma && !param.va_args) || tokens.kind == .CloseParen) &&
		    scope_level == 0) {
