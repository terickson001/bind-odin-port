...
Type :: struct {
	...
	align:      int,
	completing: bool,
	decl:       rawptr, // HACK: FIX THIS IMMEDIATELY
	variant:    union {
		Invalid,
		Incomplete,
		Primitive,
...
type_void := Type {
	size    = 0,
	variant = Primitive{.void, "void", {}},
}

type_char := Type {
	size    = size_of(c.char),
	variant = Primitive{.char, "char", {.Integer, .Unsigned}},
}

type_schar := Type {
	size    = size_of(c.schar),
	variant = Primitive{.schar, "schar", {.Integer}},
}
type_short := Type {
	size    = size_of(c.short),
	variant = Primitive{.short, "short", {.Integer}},
}
type_int := Type {
	size    = size_of(c.int),
	variant = Primitive{.int, "int", {.Integer}},
}
type_long := Type {
	size    = size_of(c.long),
	variant = Primitive{.long, "long", {.Integer}},
}
type_longlong := Type {
	size    = size_of(c.longlong),
	variant = Primitive{.longlong, "longlong", {.Integer}},
}

type_uchar := Type {
	size    = size_of(c.uchar),
	variant = Primitive{.uchar, "uchar", {.Integer, .Unsigned}},
}
type_ushort := Type {
	size    = size_of(c.ushort),
	variant = Primitive{.ushort, "ushort", {.Integer, .Unsigned}},
}
type_uint := Type {
	size    = size_of(c.uint),
	variant = Primitive{.uint, "uint", {.Integer, .Unsigned}},
}
type_ulong := Type {
	size    = size_of(c.ulong),
	variant = Primitive{.ulong, "ulong", {.Integer, .Unsigned}},
}
type_ulonglong := Type {
	size    = size_of(c.ulonglong),
	variant = Primitive{.ulonglong, "ulonglong", {.Integer, .Unsigned}},
}

type_float := Type {
	size    = size_of(c.float),
	variant = Primitive{.float, "float", {.Float}},
}
type_double := Type {
	size    = size_of(c.double),
	variant = Primitive{.double, "double", {.Float}},
}
type_longdouble := Type {
	size    = size_of(c.double),
	variant = Primitive{.longdouble, "longdouble", {.Float}},
}

type_u8 := Type {
	size    = size_of(u8),
	variant = Primitive{.u8, "u8", {.Integer, .Unsigned}},
}
type_u16 := Type {
	size    = size_of(u16),
	variant = Primitive{.u16, "u16", {.Integer, .Unsigned}},
}
type_u32 := Type {
	size    = size_of(u32),
	variant = Primitive{.u32, "u32", {.Integer, .Unsigned}},
}
type_u64 := Type {
	size    = size_of(u64),
	variant = Primitive{.u64, "u64", {.Integer, .Unsigned}},
}

type_i8 := Type {
	size    = size_of(i8),
	variant = Primitive{.i8, "i8", {.Integer}},
}
type_i16 := Type {
	size    = size_of(i16),
	variant = Primitive{.i16, "i16", {.Integer}},
}
type_i32 := Type {
	size    = size_of(i32),
	variant = Primitive{.i32, "i32", {.Integer}},
}
type_i64 := Type {
	size    = size_of(i64),
	variant = Primitive{.i64, "i64", {.Integer}},
}
...
type_size_t := Type {
	size    = size_of(c.size_t),
	variant = Primitive{.size_t, "size_t", {.Integer, .Unsigned}},
}
type_ssize_t := Type {
	size    = size_of(c.ssize_t),
	variant = Primitive{.ssize_t, "ssize_t", {.Integer}},
}
type_ptrdiff_t := Type {
	size    = size_of(c.ptrdiff_t),
	variant = Primitive{.ptrdiff_t, "ptrdiff_t", {.Integer}},
}
type_uintptr_t := Type {
	size    = size_of(c.uintptr_t),
	variant = Primitive{.uintptr_t, "uintptr_t", {.Integer, .Unsigned}},
}
type_intptr_t := Type {
	size    = size_of(c.intptr_t),
	variant = Primitive{.intptr_t, "intptr_t", {.Integer}},
}
type_wchar_t := Type {
	size    = size_of(c.wchar_t),
	variant = Primitive{.wchar_t, "wchar_t", {.Integer, .Unsigned}},
}
...
incomplete_type :: proc(decl: rawptr) -> ^Type {
	type := make_type(Incomplete{})
	type.decl = decl
	return type
}
...
complete_struct_type :: proc(incomplete: ^Type, fields: []^Type) {
	fields := fields
	if len(fields) == 0 do fields = nil
	incomplete.variant = Struct{fields}
	// @todo(Tyler): Determine struct size 
	incomplete.size = 0
}

complete_union_type :: proc(incomplete: ^Type, fields: []^Type) {
	fields := fields
	if len(fields) == 0 do fields = nil
	incomplete.variant = Union{fields}
	// @todo(Tyler): Determine struct size
	incomplete.size = 0
}
...
// 	// @todo(Tyler): Determine union size
// 	type.size = 0
// 	return type
// }
